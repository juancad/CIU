    ...
    if (!fin) {
      ...
      if ( myPort.available() > 0) {  // If data is available,
        ...
        if (val != null) {
          ...
          print("Valor sin convertir " + val);
          //print("trim: " + Float.parseFloat(val.trim()) + "\n");
          print("int(): " + int(val.trim()) + "\n");
          ...
          //mappedVal = map(valNum, 50, 650, 0, height-altojug);
          println(mappedVal);
          jug2y = int(valNum);
          fill(100, 103, 227);
          rect(jug2x, jug2y, anchojug, altojug);
          previousPos = int(valNum);
        }
      } else {
      ...
      if (goles1==5) {
        ...
        text("Pulsa SPACE para volver a jugar.\n Pulsa BACKSPACE para volver a la ventana inicial.", width/2, height/2+20);
      } else {
